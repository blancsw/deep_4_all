...
class DatasetEntrySft(DatasetEntry):
    ...
    def get_formatted(
            ...
            system_property_dropout: float = 1.0,
            system_add_length: bool = False,
    ) -> list[dict]:
        """

        :param use_system_tag: If true add system tag into the conversation (conversation metadata like lang, humour, quality)
        :param system_property_dropout: Random pick conversation metadata like lang, humour, quality. 1 mean add all properties
        :param system_add_length: Add length of the text
        :return:
        """
        output: list[dict] = []
        ...
        for i, m in enumerate(self.conversation):
            if m.role == Role.prompter:
                if use_system_tag and i + 1 < len(self.conversation):
                    ...
                    output.append({"role": "system", "content": system_tag})
                output.append({"role": "user", "content": m.text})

            else:
                output.append({"role": "assistant", "content": m.text})
...
class DatasetEntryRm(DatasetEntry):
    ...
    def get_formatted(
        ...
        :param eos_token:
        :param use_system_tag:
        :param system_property_dropout:
        :param system_add_length:
        :param max_replies:
        :return:
        """
